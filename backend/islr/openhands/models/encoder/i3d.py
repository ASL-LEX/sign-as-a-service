...
class LayersPoseLocalI3d(nn.Module):
    ...
    def __init__(
        ...
        self.max_pool = MaxPool3dSamePadding(
            kernel_size=[3, 3, 3], stride=(1, 1, 1), padding=0
        )
    ...
    def forward(self, x, yield_fts=False):
        ...
        for k in fmaps.keys():
            if k == "Logits":
                layer_vects.append(
                    fmaps[k].transpose(-2, -1)
                )  # logits already done, ths cont
                continue
    ...
    def get_final_vector(x_images, x_poses, fmaps):
        ...
        poses_center = (
            poses_center
            * torch.from_numpy(np.array([1 / ratios[2], 1 / ratios[1]])).cuda()
        )  # pose x, y, imags h, w
        ...
        pose_height = (
            ...
            .unsqueeze(1)
            .unsqueeze(-1)
            .repeat((1, ch, 1, 1, 1, wmap))
        )  # poses are x, y ===> width, height
        pose_width = (
            poses_center[:, :, :, 0:1]
            .unsqueeze(1)
            .unsqueeze(-1)
            .repeat((1, ch, 1, 1, 1, 1))
        )  # height already chosen , thusn no repeat over height needed
        ...
        pooled_maps = pooled_maps.squeeze(-1).squeeze(
            -1
        )  # removing h, w - 1 dimensions
    ...
    def _load_i3d(self, weights="", num_classes=10):
        ...
        i3d.replace_logits(
            num_classes
        )  # cause not gonna use fc layer, purpose is to feature extract
        if weights:
